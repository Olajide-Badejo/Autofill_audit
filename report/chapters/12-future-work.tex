...
\section{A smaller encoder than the one that was measured}

The transformer rung is no longer future work. It was built, fine tuned,
quantized and measured, and Section~\ref{sec:transformer-rung} has its numbers:
it cleared both accuracy bars of its pre-registered condition, missed the latency
budget by a factor of nearly three, and therefore did not ship.

What that leaves open is not whether a transformer helps on this problem, which
is now measured, but how much of the help survives shrinking. The engine that was
measured is a six layer encoder at a hidden width of seven hundred and sixty
eight, and it spends about \vBertLatencyRatio{} times the n-gram model's
arithmetic per field to buy \vBertUnseenGain{} on the held out locale and
\vBertHostileGain{} on the hostile tier. A model at roughly a quarter of that
arithmetic would sit inside the budget if it kept its accuracy.

The prediction worth registering before that measurement is that it would not
keep all of it, and the reason to take the measurement anyway is that the
accuracy it would have to keep is now a committed number rather than a guess. The
two slices to judge it on are the same two: the held out locale, where a
multilingual encoder's pretraining is the whole argument for using one, and the
hostile tier, where labels have been stripped and a bag of character n-grams has
least to work with.
