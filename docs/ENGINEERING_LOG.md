...
Faker moves to the runtime dependency set. The lock file is regenerated by
`make lock` as its own commit, per spec section 18. The pin set does not move,
because the lock was already compiled with the `dev` extra and therefore already
carried Faker at this exact version; only the annotation of which requirement
pulled it in changes.
