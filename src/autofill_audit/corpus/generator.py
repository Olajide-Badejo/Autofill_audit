...
realised grid carries eight templates per family, so two templates of one family
...
Bumped to 1.1.0 at P5R, which added three templates per family. The bytes of a
form that existed before are unchanged, because the per-form seed is a digest of
the axis tuple and none of those axes moved; what changed is the set of forms a
full run emits, and the split assignment over them. A corpus generated by 1.0.0
and one generated by 1.1.0 at the same seed are therefore not the same corpus,
which is what the version exists to say."""
